...
**Vigente (decisão do LEO, 14/09):** **isenção de GTIN** na Amazon. Solicitada dentro do fluxo de cadastro de anúncio novo e ⚠️ **RECUSADA** — motivo ainda não lido. **Hipótese de confiança ALTA:** a marca pedida (`WINNET METAIS`) não existe fisicamente no produto — o adesivo traz *"Winnet Home & Pro"* e a embalagem não tem impressão de marca, e a Amazon exige correspondência exata com a marca afixada. Se confirmado, **reenviar não resolve**: o **adesivo novo vira o caminho crítico dos três cenários** (isenção, ASIN novo e GS1). Ver `docs/CHECKLIST_ISENCAO_GTIN.md`. A filiação à GS1 fica **suspensa** — `docs/CHECKLIST_GS1_EAN.md` está marcado como suspenso, não excluído.
...
| 14/09/2026 | bloqueio | **Isenção de GTIN RECUSADA pela Amazon** (motivo não lido). Hipótese ALTA: marca `WINNET METAIS` não afixada no produto — adesivo traz "Winnet Home & Pro", embalagem sem marca, e a Amazon exige correspondência exata. **Adesivo novo vira caminho crítico dos três cenários.** Escopo da migração na O6 (22/09) passa a NÃO DECIDIR AINDA; o corte de 19/09 não será cumprido. Premissa de custo da GS1 alterada — a reimpressão passou a ser obrigatória em qualquer cenário |
