...
**Vigente (decisão do LEO, 14/09):** **isenção de GTIN** na Amazon. Solicitada dentro do fluxo de cadastro de anúncio novo e ⚠️ **RECUSADA em 14/09**. **Motivo declarado: imagens fora dos requisitos**, sem especificar qual item falhou. Duas causas abertas: **(A)** procedimento de foto — enviadas imagens de catálogo em vez de fotos reais dos 6 lados do produto e da embalagem; **(B)** a marca — o adesivo traz *"Winnet Home & Pro"* e a embalagem não tem impressão, contra a exigência de correspondência exata e marca afixada. **Teste discriminante:** refazer as fotos e reenviar. Aprovado = causa A; recusado de novo = causa B, e aí o adesivo novo é o bloqueio real. O **adesivo com a grafia `WINNET METAIS` é necessário nos três cenários** (isenção, ASIN novo, GS1) de qualquer forma. Ver `docs/CHECKLIST_ISENCAO_GTIN.md`. A filiação à GS1 fica **suspensa** — `docs/CHECKLIST_GS1_EAN.md` está marcado como suspenso, não excluído.
...
| 14/09/2026 | bloqueio | **Isenção de GTIN RECUSADA pela Amazon.** Motivo declarado: **imagens fora dos requisitos**, sem apontar o item. Duas causas abertas — (A) procedimento de foto, (B) marca não afixada (adesivo "Winnet Home & Pro", embalagem sem impressão). **Teste discriminante: refazer fotos dos 6 lados do produto e da embalagem e reenviar** — sem retoque. Corte de 19/09 segue em jogo. Adesivo com a grafia `WINNET METAIS` é necessário nos três cenários de qualquer modo; premissa de custo da GS1 alterada |
