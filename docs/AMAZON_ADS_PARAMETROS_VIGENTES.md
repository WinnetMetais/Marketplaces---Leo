...
7. **MIGRAÇÃO DE ASIN — recriação do catálogo sob a marca WINNET METAIS (pendência nova, 01/09/2026). Escopo decidido na O6 (22/09)** — adiado na O5 por decisão do LEO; gate: **isenção de GTIN** (decisão do LEO, 14/09 — substitui o GS1). Fundido com a proposta de concentração — são a mesma decisão. Insumo: P3060 + P3050 (+P4080).
   ...
   **Pré-requisito em andamento — CAMINHO TROCADO EM 14/09 (decisão do LEO):** **isenção de GTIN** na Amazon, solicitada dentro do fluxo de cadastro de anúncio novo e ⚠️ **RECUSADA em 14/09** (motivo não lido). ~~EANs próprios via GS1 Brasil (decisão Wintech + Dianna, 01/09). Isenção de GTIN descartada.~~ **Hipótese ALTA:** a marca pedida não está afixada no produto (adesivo diz "Winnet Home & Pro"; embalagem sem impressão de marca) e a Amazon exige correspondência exata com a marca afixada. **O adesivo novo passa a ser o caminho crítico** — vale igualmente para a isenção, para o ASIN novo e para a GS1. **Consequência para a O6 (22/09): o escopo da migração entra como NÃO DECIDIR AINDA**; o corte de 19/09 não será cumprido, porque reimpressão de adesivo tem lead time de produção.
